	...
	$.message = {
		...
		error : function(message,options)
		{
			...
			if(typeof options.title !== 'string')
				options.title = '<h4> Bài tập lớn MSE team 1 </h4>';
		...
		init: function(message,options)
		{
			...
			//class name
			var class_name = "alert alert-info nomargin";
			...
			if(typeof options.type ==='string')
			{
				if(options.type === "confirm")
				{
					...
					btnCancel = $('<button style="width:30%;" class="btn btn-warning" type="button">Thoát</button>');
				}
				else if(options.type === "error")
				{
					control = $('<div style="text-align:center;margin-top: 10px;"></div>');
					btnCancel =  $('<button style="width:30%;" class="btn btn-danger" type="button">Chấp nhận</button>');
					if(typeof options.button_cancel_style ==='string')
						btnCancel =  $(options.button_cancel_style);
				}
